...
foreach my $omlfile (@files) {
    unless ( $error_only ) { print "Running $omlfile ... :: ";}
    ...
    if ($result == 0) {
            $goodtests++;
            unless ( $error_only ) { print "$omlfile PASSED\n"; }
            #if ( $error_only && $goodtests % 200 == 0 ) { print "*"; }
            if ( $error_only && $goodtests % 50 == 0 ) { print "$goodtests\r"; }
        unlink $log;
    } elsif ($result == 1) {
        $int++;
